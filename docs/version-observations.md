...
| --- | --- | --- | --- | --- | --- | --- |
| 2026-07-21 | Maintainer Windows machine, sanitized read-only health report | Windows 11 Pro, x64 | `OpenAI.Codex_26.715.7063.0_x64__2p2nqsd0c76g0` | Browser `26.715.52143`; Chrome `26.715.52143` plus `latest`; Computer Use `26.715.52143` | Computer Use entry point remains `scripts\computer-use-client.mjs`; Chrome native host remains under `extension-host\windows\x64\extension-host.exe` | The unified ChatGPT desktop app still uses the `OpenAI.Codex` AppX package identity. Both expected bundled plugin files were present. A bounded 40-file scan still classified older reconcile evidence as `transient-failure-followed-by-success`; this historical evidence does not by itself indicate a current plugin failure. |
